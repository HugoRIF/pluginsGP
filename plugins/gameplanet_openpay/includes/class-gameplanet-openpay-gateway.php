...
class Gp_Openpay_Gateway extends WC_Payment_Gateway
{
  ...
  public function __construct()
  {
    ...
    $this->title = 'Pago con tarjeta de debito o credito';
  ...
  {
		update_option('gp_openpay_msi_disponibles',gp_openpay_obtener_msi_disponibilidad());
    $accept_msi = get_option('gp_openpay_msi_disponibles');
    if($accept_msi){
      echo('
          <img class="gp_openpay_icon_credit_cards" alt="" src="http://localhost/wordpress/wp-content/uploads/2022/12/Group-11.png">
      ');
    }
    else{
      echo('<img class="gp_openpay_icon_credit_cards" alt="" src="http://localhost/wordpress/wp-content/uploads/2022/12/credit_cards.png">');
    }
  }
  ...
  public function validate_fields()
  {
    ...
    if(!isset($_POST['payment_method']) && $_POST['payment_method'] != $this->id){
      ...
      wc_add_notice("Algo salió mal, inténtelo nuevamente o use otro método de pago. Si el problema continúa, comuníquese al <a href='tel:+5550475954'>+55 5047 5954</a> y lo resolveremos. CODE: GOV-001", 'error');
      return;
    }
    if(!isset($_POST['openpay-card-cvc']) && empty($_POST['openpay-card-cvc'])){
      ...
      wc_add_notice("Algo salió mal, inténtelo nuevamente o use otro método de pago. Si el problema continúa, comuníquese al <a href='tel:+5550475954'>+55 5047 5954</a> y lo resolveremos. CODE: GOV-002", 'error');
      return;
    }
    if(!isset($_POST['device_session_id']) && empty($_POST['device_session_id'])){
      ...
      wc_add_notice("Algo salió mal, inténtelo nuevamente o use otro método de pago. Si el problema continúa, comuníquese al <a href='tel:+5550475954'>+55 5047 5954</a> y lo resolveremos. CODE: GOV-003", 'error');
      return;
    }
    if(!isset($_POST['openpay_token']) && empty($_POST['openpay_token'])){
      ...
      wc_add_notice("Algo salió mal, inténtelo nuevamente o use otro método de pago. Si el problema continúa, comuníquese al <a href='tel:+5550475954'>+55 5047 5954</a> y lo resolveremos. CODE: GOV-004", 'error');
      return;
    }
    if(!isset($_POST['openpay_card_number']) && empty($_POST['openpay_card_number'])){
      ...
      wc_add_notice("Algo salió mal, inténtelo nuevamente o use otro método de pago. Si el problema continúa, comuníquese al <a href='tel:+5550475954'>+55 5047 5954</a> y lo resolveremos. CODE: GOV-005", 'error');
      return;
    }
  ...
  public function process_payment($order_id)
  {
    ...
    $card_number =  $_POST['openpay_card_number'];
    ...
    if($generate_payment_plan === false){
      gp_openpay_log($this->nonce.' process_payment', '1.2 error en los msi',json_encode($generate_payment_plan));

      wc_add_notice("Algo salió mal, inténtelo nuevamente o use otro método de pago. Si el problema continúa, comuníquese al <a href='tel:+5550475954'>+55 5047 5954</a> y lo resolveremos. Code:PP-MSI-001", 'error');
      return;
    }
  ...
  private function get_payment_plan(){
    ...
    if(!$accept_msi && isset($_POST['openpay_month_interest_free']) && !empty($_POST['openpay_month_interest_free'])){
  ...
  private function openpay_procesar_orden($device_session_id,$openpay_token,$payment_plan){
    try {
      ...
      try {
        $charge = $this->api_openpay_call("/customers/".$openpay_customer['id']."/charges",'POST',$charge_data);
        ...
        gp_openpay_log($this->nonce.' openpay_procesar_orden','3.1 No se pudo crear el cargo',$charge);
        $this->api_error($e);
        return false;
      }
  ...
  private function openpay_get_customer(){
    ...
    try {
      ...
      $cliente = $this->api_openpay_call('customers/'.$id_customer);
      return $cliente;
    } catch (Exception $e) {
        $this->api_error($e);
        return false;
  ...
  private function openpay_create_customer()
  {
    ...
    try {
      $customer = $this->api_openpay_call('customers','POST',$customer_data);
   ...
   */
  public function api_openpay_call($endpoint,$method = 'GET',$params=null){
  ...
  private function api_error(Exception $e){
    ...
    wc_add_notice("Algo salió mal, inténtelo nuevamente o use otro método de pago. Si el problema continúa, comuníquese al <a href='tel:+5550475954'>+55 5047 5954</a> y lo resolveremos. Code: GPOP-". $e->getCode(), 'error');
